      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid gap-10 md:grid-cols-4">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            {[Phone, MapPin, Mail].map((Icon,i) => (
              <a key={i} href="#" className="w-9 h-9 rounded-full bg-white/[0.07] flex items-center justify-center text-gray-400 hover: hover:bg-white/[0.11] transition-all" >
                <Icon size={15} />
              </a>
            ))}
          </div>
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm text-gray-500">
            {['/','/story','/menu','/shop','/contact'].map(to=>(
              <li key={to}><Link to={to} className="hover: transition-colors" >{to==='/'?'Home':to.slice(1).replace(/^./,c=>c.toUpperCase())}</Link></li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-gray-500">
            <li className="flex gap-2.5"><MapPin size={14} className="mt-0.5" />Tamraght, Morocco</li>
            <li className="flex gap-2.5"><Phone size={14} className="mt-0.5" />+212 528 000 000</li>
            <li className="flex gap-2.5"><Mail size={14} className="mt-0.5" />hola@kinomori.ma</li>
          </ul>
        </div>
