...
import { ArrowDownRight, Star, Quote, ShoppingBag, Instagram, Facebook, Phone, MapPin, Mail } from 'lucide-react';
import { useTranslation } from 'react-i18next';

// ── Shader de fondo estilo ocean-night ───────────
function ShaderPlane() {
  const ref = useRef();
  const mouse = useRef({ x:.5, y:.5 });
  const u = useRef({ time:0, mouse:[.5,.5] });
  const { size } = useThree();
  ...
  useFrame((_, delta) => {
    u.current.time += delta;
    u.current.mouse[0] += (mouse.current.x - u.current.mouse[0]) * .04;
    u.current.mouse[1] += (mouse.current.y - u.current.mouse[1]) * .04;
    if (ref.current) {
      (ref.current.material as any).uniforms.uTime.value = u.current.time;
      (ref.current.material as any).uniforms.uMouse.value = u.current.mouse;
    }
  });
...
function Particles() {
  ...
  useFrame((_,d) => { if(ref.current) ref.current.rotation.y += d * .008; });
  return (
    <points ref={ref}>
      <bufferGeometry>
        <bufferAttribute attach="attributes-position" args={[positions, 3]} />
      </bufferGeometry>
      <pointsMaterial color="#f59e0b" size={.07} transparent opacity={.55} />
    </points>
  );
}

// ── Forma flotante genérica ───────────────────────
function Shape({ fin, pin, col, geo }: { fin:[number,number,number], pin:[number,number,number], col:number[], geo:any }) {
  const ref = useRef<any>();
  useFrame((_,d) => {
    if(!ref.current) return;
    ref.current.rotation.y += d * .4 * (fin[0]+1);
    ref.current.rotation.x += d * .2 * (fin[1]+1);
    ref.current.position.y = pin[1] + Math.sin(performance.now()*.001 * fin[2]) * .25;
  });
  return (
    <Float speed={2} floatIntensity={.8}>
      <mesh ref={ref} position={pin}>
        <primitive object={geo} attach="geometry" />
        <meshStandardMaterial color={col} metalness={.7} roughness={.25} />
      </mesh>
    </Float>
  );
}
      ...
      <section className="relative h-[100svh] w-full overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Canvas camera={{ position:[0,0,18], fov:55 }} dpr={[1,2]}>
            ...
            <ShaderPlane />
            <Particles />
            <Shape fin={[1,.5,.8]} pin={[-3.5,1.5,-5]} col={[.1,.18,.1]} geo={{
              attributes:{position:{type:'Float32Array',value:new Float32Array([0,0,0])}},
              get index(){ return null; }
            }} />
            <Shape fin={[1.2,.6,1]} pin={[3.8,-1,-6]} col={[.96,.62,.04]} geo={{ get index(){ return null; }, get attributes(){ return {}; } }} />
          </Canvas>
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-10">
          <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} transition={{duration:.9}}
            className="mb-5 px-6 py-2 rounded-full text-[11px] font-black tracking-[.32em] uppercase border border-kin-gold/30 text-kin-gold/90 backdrop-blur-md bg-kin-gold/[0.06]">
            KINOMORI · {t('hero.tagline')}
          </motion.div>
          <motion.h1 initial={{opacity:0,y:50}} animate={{opacity:1,y:0}} transition={{duration:1.1,delay:.12}}
            className="font-display text-[clamp(3.5rem,10vw,9rem)] font-black text-white tracking-[.04em] leading-[0.88]"
            style={{textShadow:'0 1px #ffffff0f, 0 2px 30px #000000a6, 0 8px 80px #0006'}}>
            {t('hero.title')}
          </motion.h1>
          ...
          <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.85,delay:.52}}
            ...
            <a href="#contact"
              className="px-9 py-4 rounded-full bg-kin-gold hover:bg-amber-400 text-kin-dark font-bold text-base transition-all shadow-[0_8px_32px_rgba(245,158,11,.55)] hover:shadow-[0_14px_48px_rgba(245,158,11,.75)] hover:-translate-y-0.5 active:scale-[.97] duration-300">
              {t('hero.cta_reserve')}
            </a>
            <a href="#menu"
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:.4}} transition={{delay:1.25,duration:1}}
